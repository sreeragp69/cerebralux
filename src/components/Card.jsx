...
const Card = React.memo(({ title, description, image, link }) => {
  ...
  return (
    ...
    >
      ...
      <div className="w-full lg:w-1/2 px-7 py-6 flex flex-col justify-between relative">
        ...
        <Link to={link}>
          <button className="mt-6 text-sm text-white border border-white px-4 py-1.5 rounded-full hover:bg-white hover:text-[#ccb28e] transition-all duration-300 w-fit self-end">
            Learn More
          </button>
        </Link>
      </div>
